...
module LanguagePack
  module PackageFetcher
    ...
    def fetch_mvn_package(version)
      mvn_package = packages_config["mvn"].find { |p| p["version"] == version }

      raise "Unsupported Mvn version: #{version}" unless mvn_package

      #fetch_from_buildpack_cache(jdk_package["mvn"]) ||
      #fetch_from_blobstore(jdk_package["jre"]) ||
      fetch_from_curl(jdk_package["full"], VENDOR_URL)
    end
    ...
    def fetch_from_curl(filename, url)
      puts "package_fetch.rb ---Downloading #{filename} from #{url} ..."
